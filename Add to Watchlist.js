...
// icon-color: deep-purple; icon-glyph: plus-circle;
...
const locationArray = ["Plex", "iTunes", "Netflix", "Prime", "Hulu", "HBO", "Showtime", "Vudu", "Hoopla", "Filmstruck", "N/A"];
...
var whichCategory = new Alert();
whichCategory.title = "Which category should this go to?";
const categoryArray = ["Main List", "Must See", "Shortlist", "2019"];
for (i in categoryArray){
	let category = categoryArray[i];
	whichCategory.addAction(category);	
}
var category = await whichCategory.present();
...
airReq.method = "POST";
var airFields = { "Name": nameYear(movie), "Category": category, "Location": location, "Runtime": movie["runtime"], "Metascore": Number(omdbRaw["Metascore"]), "Genres": getGenres(movie), "TMDB ID": movie["id"].toString(), "IMDB ID": movie["imdb_id"], "4K": isFourK, "Comment": comment };
...
// Now we gotta send this to TMDB
var tmdbWatchURL = tmdbURL + "3/account/" + tmdbUser + "/watchlist?api_key=" + tmdbAPI + "&session_id=" + tmdbSession;
var tmdbSend = new Request(tmdbWatchURL);
tmdbSend.method = "POST";
tmdbSend.headers = {"Content-Type": "application/json;charset=utf-8"}
var watchBody = { "media_type": "movie", "media_id": Number(movie["id"]), "watchlist": true };
tmdbSend.body = JSON.stringify(watchBody);
var responseTwo = await tmdbSend.loadJSON();
