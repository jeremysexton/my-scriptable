...
var sortAsk = new Alert();
var sortTypes = ["Quick", "This Year", "Action", "Comedy", "Crime", "Mystery", "Horror", "Thriller", "Critically Acclaimed"];
for (i in sortTypes) {
  sortAsk.addAction(sortTypes[i]);
}
sortAsk.title = "Whatcha lookin for?";
var sortAns = await sortAsk.present();
var selected = sortTypes[sortAns];

if (sortAns == 0) {
	var formula = "{Runtime} < 110";
	var airURL = "https://api.airtable.com/v0/appykEQk3PKmo9whh/Movies?view=Grid%20view&pageSize=50&maxRecords=200&filterByFormula=" + encodeURI(formula);
} else if (sortAns == 1) {
  var formula = "{Category} = 2019";
  var airURL = "https://api.airtable.com/v0/appykEQk3PKmo9whh/Movies?view=Grid%20view&pageSize=50&maxRecords=200&filterByFormula=" + encodeURI(formula);
} else {
	var airURL = "https://api.airtable.com/v0/appykEQk3PKmo9whh/Movies?pageSize=50&maxRecords=200&view=" + encodeURI(selected);
}
